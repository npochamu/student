...
<c:if test="${not empty errorMessages}">
    <span style="color: red;">
        <c:out value="${fn:substring(errorMessages, 0, fn:length(errorMessages) - 1)}" />を選択してください。
    </span>
</c:if>
</span>
			...
			<c:choose>
							...
							<c:forEach var="test" items="${students}" varStatus="status">
								<tr>
									...
									<td class="py-1"><input type="text"
										id="point${status.count}" name="point${status.count}"
										value="${test.getPoint_str()}" class="form-control"
										pattern="^(100|[1-9]?[0-9])$" title="0～100の範囲で入力してください"
										></td>
								</tr>
								<input type="hidden" name="stucd${status.count}"
									value="${test.getStudent_No()}">
								<input type="hidden" name="clsno${status.count}"
									value="${test.getClass_Num()}">
							</c:forEach>
						</table>
				...
				<c:when
				test="${students.size()==0 and empty errorMessages}">
